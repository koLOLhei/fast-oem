...
export function ReceiptButton({ orderId, token, defaultName }: Props) {
    ...
    const handleDownload = () => {
        ...
        const url = `/api/receipts/${orderId}?token=${encodeURIComponent(token)}&addressee=${encodeURIComponent(addressee.trim())}&reissue=true`
    ...
    if (!open) {
        return (
            <button
                onClick={handleOpen}
                className="shrink-0 px-4 py-2 bg-primary text-primary-foreground text-sm font-medium rounded-lg hover:bg-primary/90 transition"
            >
                📄 領収書PDF
            </button>
        )
        ...
        <div className="shrink-0 space-y-2 min-w-[220px]">
            ...
            <div className="flex gap-2">
                <button
                    ...
                    className="flex-1 px-3 py-1.5 bg-primary text-primary-foreground text-sm font-medium rounded-lg hover:bg-primary/90 transition"
                >
                    📄 発行する
                </button>
                <button
