...
export async function sendAllShippedNotification({
  ...
  const itemRowsHtml = items
    .map(
      (item) => `
        <tr>
          <td style="padding:12px 14px; border-bottom:1px solid #e5e7eb; font-size:13px; color:#1f2937;">${item.productName}</td>
          ...
          <td style="padding:12px 14px; border-bottom:1px solid #e5e7eb; font-family:monospace; font-weight:bold; font-size:14px; color:#1e3a8a; letter-spacing:0.04em;">${item.trackingNumber}</td>
        </tr>`
    )
    .join('')
      ...
      <div style="font-family:sans-serif; max-width:600px; margin:0 auto; padding:0; background:#f9fafb;">
        ...
        <div style="background:#ffffff; padding:32px; border-radius:0 0 8px 8px; border:1px solid #e5e7eb; border-top:none;">
          ...
          <div style="padding:14px 16px; background:#f3f4f6; border-radius:8px; margin-bottom:24px;">
            ...
            <p style="margin:4px 0 0; font-family:monospace; font-weight:700; font-size:16px; color:#1f2937;">${orderNumber}</p>
          </div>
